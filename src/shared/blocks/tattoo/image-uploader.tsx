  ...
  if (photoUrl) {
    return (
      <div className="group relative aspect-square w-full overflow-hidden rounded-lg border border-border/50 bg-muted">
        ...
        <img src={photoUrl} alt="Body photo preview" className="h-full w-full object-contain" />
        {!disabled && (
          <button
            type="button"
            onClick={onClear}
  ...
  return (
    <div
      ...
      className={[
        ...
        dragOver ? 'border-primary bg-primary/5' : 'border-border/60 bg-muted/30 hover:border-border hover:bg-muted/60',
    ...
    >
      ...
      {uploading ? (
        ...
        <>
          ...
          <p className="text-xs text-muted-foreground">or click to browse · max {MAX_MB}MB · JPG/PNG/WebP</p>
        </>
      )}
      {localError && <p className="mt-1 text-xs text-destructive">{localError}</p>}
      <input
        ref={inputRef}
        type="file"
