...
// Tattoo AI business tables (from Demo migration)
export const tattooProject = table(
  'tattoo_project',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
      ...
      .references(() => user.id, { onDelete: 'cascade' }),
    prompt: text('prompt').notNull(),
    bodyPhotoKey: text('body_photo_key').notNull(),
    bodyPhotoUrl: text('body_photo_url').notNull(),
    tattooDesignKey: text('tattoo_design_key'),
    tattooDesignUrl: text('tattoo_design_url'),
  ...
  (table) => [
    index('idx_tattoo_project_user_id').on(table.userId, table.status),
    index('idx_tattoo_project_created_at').on(table.createdAt),
  ]
);

export const tattooGeneration = table(
  'tattoo_generation',
  {
    id: text('id').primaryKey(),
    projectId: text('project_id')
      .notNull()
      .references(() => tattooProject.id, { onDelete: 'cascade' }),
  ...
  (table) => [
    index('idx_tattoo_generation_project_id').on(table.projectId),
  ]
);

// Guest free usage tracking (IP-based, 3 per day limit)
export const guestFreeUsage = table(
  'guest_free_usage',
  {
    ip: text('ip').notNull(),
  ...
  (table) => [
    primaryKey({ columns: [table.ip, table.usedDate] }),
  ]
);
